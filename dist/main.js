...
(() => {
  ...
  var K = /[\s\p{P}\p{S}]/;
  var ae = /[^\s\p{P}\p{S}]/;
  var Be = k(/^((?![*_])punctSpace)/, "").replace(/punctSpace/g, K).getRegex();
  var le = /(?!~)[\p{P}\p{S}]/;
  var De = /(?!~)[\s\p{P}\p{S}]/;
  var qe = /(?:[^\s\p{P}\p{S}]|~)/;
  var ue = /(?![*_])[\p{P}\p{S}]/;
  ...
  var Ze = k(pe, "").replace(/punct/g, v).getRegex();
  var Ne = k(pe, "").replace(/punct/g, le).getRegex();
  var ce = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)";
  var Qe = k(ce, "g").replace(/notPunctSpace/g, ae).replace(/punctSpace/g, K).replace(/punct/g, v).getRegex();
  var je = k(ce, "g").replace(/notPunctSpace/g, qe).replace(/punctSpace/g, De).replace(/punct/g, le).getRegex();
  var Fe = k("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "g").replace(/notPunctSpace/g, ae).replace(/punctSpace/g, K).replace(/punct/g, v).getRegex();
  var Ue = k(/^~~?(?:((?!~)punct)|[^\s~])/, "").replace(/punct/g, ue).getRegex();
  var Ke = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)";
  var We = k(Ke, "g").replace(/notPunctSpace/g, He).replace(/punctSpace/g, ve).replace(/punct/g, ue).getRegex();
  var Xe = k(/\\(punct)/, "g").replace(/punct/g, v).getRegex();
...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  var getTheme = () => dark$.matches ? "dark" : "light";
  var updateTheme = () => {
  ...
  $input.oninput = $input.onchange = $input.onpaste = () => {
    $markdown.innerHTML = $source.textContent = g($input.value, { async: false });
  ...
  var example = "\u8FD9\u662F\u4E00\u4E2A**\u201C\u4F1A\u5F15\u8D77\u201D**\u6E32\u67D3\u9519\u8BEF\u7684**\u201C\u5DF2\u77E5\u95EE\u9898\u201D**\uFF0C\u5F53\u52A0\u91CD\u7B26\u53F7\\*\\*\u9047\u5230\u67D0\u4E9B\u4E2D\u6587\u6807\u70B9\u65F6\uFF0C\u53EF\u80FD\u5C31\u4F1A\u51FA\u73B0**\u201C\u8BC6\u522B\u4E0D\u4E86\u201D**\u7684\u60C5\u51B5\u3002\u5C31\u5982\u8FD9\u53E5\u8BDD\u5C55\u73B0\u7684\u4E00\u6837\u3002";
  ...
  var init = new URL(location.href).searchParams.get("text");
  if (init) {
    $input.value = init;
  }
  $input.dispatchEvent(new InputEvent("change"));
